...
def find_new_path(recv_data, shortestPath):
	...
	k=0
	for i in range(comma_idx-1,open_brack_idx,-1):
		obstacle_x = obstacle_x+int(recv_data[i])*(10**k)
		k=k+1
	obstacle_x = int(obstacle_x)

	k=0
	for i in range(close_brack_idx-1,comma_idx,-1):
		obstacle_y = obstacle_y+int(recv_data[i])*(10**k)
		k=k+1
