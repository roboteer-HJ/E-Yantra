...
int socket_create(struct sockaddr_in dest_addr, struct sockaddr_in source_addr){
	...
	listen(sock,3);
	int adrsz = sizeof(dest_addr);
	my_sock = accept(sock,(struct sockaddr *)&dest_addr, (socklen_t*)&adrsz);
...
int receive_from_send_to_client(int sock){
	...
	while(line_data[0]<ch)
	{
	fgets(line_data, MAXCHAR, input_fp);
	fgets(line_data, MAXCHAR, input_fp);
	}
	start = start+3;
	if(start<len-1 && line_data[0]==ch)
	{
	tx_buffer[0] = '@';
        for(int i=start;;i++)
        {
         tx_buffer[c] = line_data[i];
	c++;
	if(line_data[i]==')')
	{
	tx_buffer[c] = '@';
